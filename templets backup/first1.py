...
def doctor_profile_signup():
    if request.method == 'POST':
        ...
        role = 'doctor'  # إذا كنت تريد تحديده مباشرة هنا، أو يمكنك أخذه من الـ form أيضًا
        profile_picture = ''
        ...
        if role.lower() == 'doctor':
            return redirect(url_for('doctor_details'))  # صفحة doctor_details على سبيل المثال
        elif role.lower() == 'patient':
            return redirect(url_for('patient_details'))  # صفحة patient_details
...
def patient_profile_signup():
    if request.method == 'POST':
        ...
        flash('Account created successfully!', 'success')
        return redirect(url_for('patient_details'))  # Redirect to avoid resubmitting the form if the page is refreshed
...
def patient_details():
    return render_template('patient_details.html')  # تأكد من أنك تعرض الصفحة الصحيحة
